...
def get_full_retirements_additions_per_iso(location,plants,planned,full_tech_list,date_of_last_report, ISO): ## PJM, ISNE, NYIS
    ...
    additions,approved_additions,not_yet_approved_additions = RGGI_capacity(location).analyse_RGGI_planned_capacity_PJM(ISO_planned)
    iso_plus_retirements_plus_additions_timeseries, iso_time_series_capacity_fig = RGGI_capacity(location).estimated_timeseries_capacity_PJM(iso_plants_analysed,full_tech_list,additions,date_of_last_report,PJM_retiredates=True)
    
    return iso_plus_retirements_plus_additions_timeseries, iso_time_series_capacity_fig

def run_historical_RGGI_share(location,read_latest):
    time_series_historical_capacity_with_tech = RGGI_capacity(location).save_historical_capacity_per_plant(years_back=6,read_latest=read_latest)
...
def run_future_RGGI_share(location):
    next_update_time,recent_report,report_month,report_year,date_of_last_report  =RGGI_capacity(location).scrape_recent_EIA_860m(lagged_report=2)
    ...
    plants['PJM_tech'] = plants['Technology'].map(tech_convert_dict)
    plants['Fossil'] = np.where(plants['PJM_tech'].isin(Fossil_tech),1,0)
    dual_fuels = plants.groupby(['Plant ID','report_month','report_year'])['PJM_tech'].nunique()
    plants = plants.join(dual_fuels.to_frame('dual_fossil'),on=['Plant ID','report_month','report_year'])
    plants['PJM_tech'] = np.where(plants['dual_fossil']>1,'Multiple Fuels',plants['PJM_tech'])
    full_tech_list = plants.PJM_tech.unique()
    PJM_plants =  plants.query(f'`Balancing Authority Code`=="PJM" ')
    PJM_plants = RGGI_capacity(location).analyse_RGGI_capacity(PJM_plants)
    ...
    additions,approved_additions,not_yet_approved_additions = RGGI_capacity(location).analyse_RGGI_planned_capacity_PJM(PJM_planned)
    pjm_rggi_additions,pjm_rggi_approved_additions,pjm_rggi_not_yet_approved_additions = RGGI_capacity(location).analyse_RGGI_planned_capacity_PJM(RGGI_PJM_planned)

    PJM_plants['rggi_state'] = np.where(PJM_plants['Plant State'].isin(RGGI_capacity(location).RGGI_states),1,0)

    PJM_RGGI_plants = PJM_plants.query('rggi_state==1 and `Nameplate Capacity (MW)`>25 ') #
    pjm_plus_retirements_plus_additions_timeseries, pjm_time_series_capacity_fig = RGGI_capacity(location).estimated_timeseries_capacity_PJM(PJM_plants,full_tech_list,additions,date_of_last_report,PJM_retiredates=True)
    
    rggi_pjm_rggi_plus_retirements_plus_additions_timeseries, rggi_pjm_time_series_capacity_fig = RGGI_capacity(location).estimated_timeseries_capacity_PJM(PJM_RGGI_plants,full_tech_list,pjm_rggi_additions,date_of_last_report,PJM_retiredates=True)

    pjm_plus_retirements_plus_additions_timeseriesv2, pjm_time_series_capacity_fig = get_full_retirements_additions_per_iso(location,plants,planned,full_tech_list,date_of_last_report, ISO='PJM')
    isne_plus_retirements_plus_additions_timeseries, isne_time_series_capacity_fig = get_full_retirements_additions_per_iso(location,plants,planned,full_tech_list,date_of_last_report, ISO='ISNE')
    nyis_plus_retirements_plus_additions_timeseries, nyis_time_series_capacity_fig = get_full_retirements_additions_per_iso(location,plants,planned,full_tech_list,date_of_last_report, ISO='NYIS')
    ...
    return rggi_pjm_rggi_plus_retirements_plus_additions_timeseries,pjm_plus_retirements_plus_additions_timeseries,PJM_plants,pjm_plus_retirements_plus_additions_timeseriesv2,isne_plus_retirements_plus_additions_timeseries,nyis_plus_retirements_plus_additions_timeseries

def run_full_relative_capacity_PJM(location,read_latest):
    historical_rggi_share_pjm,test_df = run_historical_RGGI_share(location,read_latest=read_latest)
    historical_rggi_share_pjm['RGGI_share'] = historical_rggi_share_pjm[1]/historical_rggi_share_pjm[[1,0]].sum(axis=1)
    rggi_pjm_rggi_plus_retirements_plus_additions_timeseries,pjm_plus_retirements_plus_additions_timeseries,PJM_plants,pjm_planned_retired,isne_planned_retired,nyis_planned_retired =  run_future_RGGI_share(location)
    ...
    return historical_and_forecast,pjm_planned_retired,isne_planned_retired,nyis_planned_retired 